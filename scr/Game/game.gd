...
func _ready():
	...
	enemy_manager_instance.name = "EnemyManager"  # чтобы его можно было найти по имени, если потребуется
...
func spawn_player():
	...
	add_child(player)
	var spawn_position = root_node.get_room_center(1) * tile_size # Устанавливает позицию персонажа в центр первой комнаты
